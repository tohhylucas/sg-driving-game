...
- [DONE] Validate a browser-authored donut `mapData.json` from the map editor in the game runtime. Delivery note: Generated a donut source PNG, exported and re-imported a 33-point donut `mapData.json` through the editor in Chrome, added an opt-in `?mapData=/maps/...` game preview renderer for editor road paths/markings, and verified the donut JSON renders in the live game cockpit without changing the default fixed route.
- [DONE] Document the full map editor authoring and game preview workflow. Delivery note: Rewrote the map editor README as a step-by-step guide covering setup, image calibration, road tracing, symbols, export/import, `public/maps` placement, `?mapData=/maps/...` game preview, diagnostics, verification, and troubleshooting.
- [DONE] Verify editor symbols and non-center origins in game map previews. Delivery note: Added game-side rendering for editor-exported road-symbol decals, covered all symbol types plus shifted-origin round trips with tests, added a reusable symbol/origin smoke map under `public/maps/`, and verified it in the browser preview.
- [DONE] Add editor-authored scenery and kerb preview geometry. Delivery note: Added tree/grass placement, traced kerb-line authoring, JSON export/import support for `scenery` and `kerbLines`, game preview rendering with vertical scenery and raised alternating black-white kerb bricks, smoke JSON, tests, builds, and browser screenshots.
- [DONE] Naturalize editor-authored grass rendering in the game preview. Delivery note: Replaced blocky grass cuboids with deterministic tuft geometry using a ground patch plus varied tapered blades, covered the scene graph in tests, and verified the scenery smoke map in Chrome with screenshot evidence.
